...
$(LOCALBIN)/$(ARENA): $(LOCALBIN)
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -tags netgo -ldflags '${LDFLAGS}' -o $(LOCALBIN)/$(ARENA) cmd/arena/main.go
...
$(ARENA_INSTALLER_TARBALL): arena kubectl helm
	...
	cp uninstall.sh $(TEMPDIR)/$(ARENA_INSTALLER)/bin/arena-uninstall.sh && \
